...
const PlanCard = ({ planIndex, form, canRemove, communityData, onRemove, onAddFeature, onRemoveFeature, onUpdatePlan }: PlanCardProps) => {
    ...
    return (
        <div className="flex flex-col gap-4 p-4 rounded-2xl bg-gray-800/30 border border-gray-700">
            ...
            <PlanFeatures
                ...
                onAddFeature={() => onAddFeature(planIndex)}
                onRemoveFeature={(featureIndex) => onRemoveFeature(planIndex, featureIndex)}
            />
        </div>
    )
}
