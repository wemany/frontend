...
const RoleHitoField = ({ control, onAddHito, updateHito, onRemoveHito, getHitosValues }: HitoFieldProps) => {
    ...
    const uniquePlugins = Array.from(new Set(availableMilestones.map(m => m.pluginId)))
        .map(pluginId => {
            const milestone = availableMilestones.find(m => m.pluginId === pluginId);
                    ...
                    <div>
                        ...
                        <div className="space-y-4">
                            {(field.value || []).map((milestone, index) => {
                                const selectedPluginMilestones = availableMilestones.filter(m => m.pluginId === milestone.plugin);
