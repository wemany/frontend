...
const CardPlan = ({ plan, roles, onEdit, onDelete }: CardPlanProps) => {
    return (
        <motion.div
            key={plan.id}
            ...
            className={`relative overflow-hidden rounded-xl bg-gradient-to-br from-${plan.color}-600/10 to-${plan.color}-600/10`}
        >
            <Card className={`px-4 py-6 border-${plan.color}-500/30 h-full w-full flex flex-col justify-between`}>
                <CardHeader className="text-center gap-0 ml-10">
                    <CardTitle className="text-2xl font-bold mb-2">
                        <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4 bg-${plan.color}-600/20`} >
                            <DollarSign className={`h-8 w-8 text-${plan.color}-400`} />
                        </div>
                        <h3>{plan.name}</h3>
                    </CardTitle>
