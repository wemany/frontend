...
const PlansListManager = ({ plans, roles, setOpenModal, onEditPlan, onDeletePlan }: PlansListManagerProps) => {
        ...
        <>
            {plans.length === 0 ? <CreateNew handleCreate={() => setOpenModal(true)} title="Crear Nuevo Plan" description="Crea un nuevo plan para tu comunidad" /> :
                <div className={containerClasses}>
                    {plans.map(plan => (
                        <CardPlan key={plan.id} plan={plan} roles={roles} onEdit={onEditPlan} onDelete={onDeletePlan} />
                    ))}
                </div>}
        </>
    );
